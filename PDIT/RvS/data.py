...
# This is used from the official implementation.
datasets = [
...
class EpisodeDataset(Dataset):
    def __init__(self, config):
        ...
        self.image_dim = (7, 7, 3)
        self.act_dim = 6
        
        dataset_path = f"./data"
        self.demos = load_demos(dataset_path, datasets, None)
    ...
    def __getitem__(self, index):
        ...
        s = torch.from_numpy(s).to(dtype=torch.float16, device=self.device)
        m = torch.from_numpy(m).to(dtype=torch.long, device=self.device)
        m_mask = torch.from_numpy(m_mask).to(dtype=torch.float16, device=self.device)
        a = torch.from_numpy(a).to(dtype=torch.float16, device=self.device)
        # r = torch.from_numpy(r).to(dtype=torch.float16, device=self.device)
        # d = torch.from_numpy(d).to(dtype=torch.long, device=self.device)
        rtg = torch.from_numpy(rtg).to(dtype=torch.float16, device=self.device)
        timesteps = torch.from_numpy(timesteps).to(dtype=torch.long, device=self.device)
        mask = torch.from_numpy(mask).to(dtype=torch.float16, device=self.device)
        
        return {"images": s, "prompt": m, "prompt_mask": m_mask, "actions": a, "Gt": rtg, "timesteps": timesteps, "mask": mask}
...
def transform_demos(demos):
    ...
    for demo in demos:
        ...
        directions = demo[2]
        actions = demo[3]
        all_images = blosc.unpack_array(all_images).astype(np.uint8)
